...
namespace UIWidgets {
	...
	public class SlideBlock : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
		...
		{
			var handle = handleObject.GetComponent<SlideBlockHandle>() ?? handleObject.AddComponent<SlideBlockHandle>();
		...
		void Animate()
		{
			...
			currentAnimation = RunAnimation(Time.time, animationLength, startPosition, movement, acceleration);
			StartCoroutine(currentAnimation);
		}

		IEnumerator RunAnimation(float startTime, float animationLength, float startPosition, float movement, float acceleration)
		{
			float delta;
